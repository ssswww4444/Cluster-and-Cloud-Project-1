...
def read_tweet_single(tweet_file, grid_data):
    """ Read twitter data from file and process data """
    ...
    with open(tweet_file, "r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            grid_hashtag_dict[grid] += hashtags
...
def read_tweet_master(comm, tweet_file, size):
    """ Read twitter data from file and send to processes """
    ...
    with open(tweet_file, "r", encoding="utf-8") as f:
        ...
        for i, line in enumerate(f):
            ...
            if line[0] != "{":
                continue
            tweet = json.loads(fix_line(line))
            # send tweet data to each slave
            req = comm.isend(tweet, dest=i%(size-1)+1)
            requests.append(req)
        f.close()

    # tell all slaves to finish
    for i in range(1,size):
        req = comm.isend("END", dest=i)
	req.append(req)

    end_time = time.time()
    print("Reading file time: ", end_time - start_time, " seconds")
    MPI.Request.Waitall(requests)
            

def process_tweet_slave(comm, grid_data):
...
def main():
    ...
    if comm_size == 1:
        # Single Core
        reduced_post_count, combined_hashtag_dict = read_tweet_single(args.tweet_file, grid_data)
    else:
        # Multi Core
        if comm_rank == 0:
            # master
            read_tweet_master(comm, args.tweet_file, comm_size)

            # empty results
            grid_post_count = np.zeros(len(grid_data))
            grid_hashtag_dict = {}
            for i in range(len(grid_data)):
                grid_hashtag_dict[i] = Counter()
            ...
            # slaves
            grid_post_count, grid_hashtag_dict = process_tweet_slave(comm, grid_data)
